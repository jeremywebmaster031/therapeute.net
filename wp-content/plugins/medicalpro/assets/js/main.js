...
jQuery(document).ready(function () {
    ...
    jQuery(document).on('click', '.PayoutMethodModalFormSubmit', function (e) {
        ...
        var $this = jQuery(this),
			Form = jQuery('.PayoutMethodModalForm'),
            FD = Form.serialize(),
            type = jQuery('.PayoutMethod_container.active').data('value'),
			accountTitle = Form.find('#accountTitle'),
			accountNumber = Form.find('#accountNumber'),
			error = false;

		if (accountTitle.val() === '') {
			accountTitle.addClass('error');
			error = true;
		}else {
			accountTitle.removeClass('error');
		}
    ...
    jQuery(document).on('change','#mp-dashboard-timeslot', function() {
        ...
        jQuery.ajax({
            ...
            url: ajax_search_term_object.ajaxurl,
            data:{
                'action':'mp_booking_timeSlot_duration',
	...
	//New by abbas
	jQuery('div#mp-location-tab .mp-profile-location-booking-container button.mp-profile-location-book.book-appoinment-btn').click(function() {
        jQuery('html, body').animate({
               scrollTop: jQuery('div#mp-profile-sidebar-mobile').offset().top
        }, 2000);
    });
	//End new by abbas
