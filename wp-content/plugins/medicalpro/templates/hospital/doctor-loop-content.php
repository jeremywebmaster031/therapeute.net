...
$latitude = listing_get_metabox_by_ID('latitude', $doctor_id);
$longitude = listing_get_metabox_by_ID('longitude', $doctor_id);
$gAddress = listing_get_metabox_by_ID('gAddress', $doctor_id);
$phone = listing_get_metabox_by_ID('phone', $doctor_id);
$isfavouriteicon = listingpro_is_favourite_grids($doctor_id, $onlyicon = true);
$isfavouritetext = listingpro_is_favourite_grids($doctor_id, $onlyicon = false);

$adStatus = get_post_meta($doctor_id, 'campaign_status', true);
$CHeckAd = '';
$adClass = '';
if ($adStatus == 'active') {
...
$certified_doctor = get_post_meta($doctor_id, 'mp_listing_extra_fields_certified_doctor', true);
...
<div class="col-md-12 mp-hospital-archive <?php echo esc_attr($adClass); ?> lp-grid-box-contianer grid_view_s2 grid_view2 card1 lp-grid-box-contianer1 listing-grid-view2-outer md-listing-outer md-listing-outer-list"
     data-title="<?php echo get_the_title($doctor_id); ?>" data-postid="<?php echo $doctor_id; ?>"
     data-lattitue="<?php echo esc_attr($latitude); ?>" data-longitute="<?php echo esc_attr($longitude); ?>"
     data-posturl="<?php echo get_the_permalink($doctor_id); ?>">
    <?php if (is_page_template('template-favourites.php')) { ?>
        <div class="remove-fav md-close" data-post-id="<?php echo $doctor_id; ?>">
        ...
        <div class="lp-grid-box-thumb">
            <div class="show-img">
                ...
                if (has_post_thumbnail($doctor_id)) {
                    $image = wp_get_attachment_image_src(get_post_thumbnail_id($doctor_id), 'full');
                    if (!empty($image[0])) {
                        echo "<a href='" . get_the_permalink($doctor_id) . "' >
																<img src='" . $image[0] . "' />
															</a>";
                    } else {
                        echo '
														<a href="' . get_the_permalink($doctor_id) . '" >
    ...
    <div class="md-list-box-content-container clearfix">
        <div class="pull-left">
            <div class="lp-grid-box-description">
                ...
                <div class="lp-grid-box-left">
                    <h4 class="lp-h4">
                        <a href="<?php echo get_the_permalink($doctor_id); ?>">
                            <?php echo mb_substr(get_the_title($doctor_id), 0, 40); ?>
                            <?php echo $claim; ?>
                        </a>
                    </h4>
                    <ul class="md-loop-cat margin-top-5">
                        <li>
                            <?php
                            $cats = get_the_terms($doctor_id, 'listing-category');
                            if (!empty($cats)) {
                                foreach ($cats as $cat) {
                ...
                <div class="md-listing-stars-main-container clearfix">
                    <?php
                    $rating = get_post_meta($doctor_id, 'listing_rate', true);
                    $rating_num_bg = '';
                    $rating_num_clr = '';
                    if ($rating < 3) {
                ...
                <?php
                $plan_id = listing_get_metabox_by_ID('Plan_id', $doctor_id);
                $tagline_show = get_post_meta($plan_id, 'listingproc_tagline', true);
                if ($plan_id == 'none' || empty($plan_id)) :
                    $tagline_show = true;
                endif;
                $tagline_text = listing_get_metabox_by_ID('tagline_text', $doctor_id);
                ...
                <div class="">
                    <?php if (!empty($gAddress)) { ?>
                        <div class="">
                            <span class="text gaddress" title="<?php echo $gAddress; ?>"><i class="fa fa-map-marker" aria-hidden="true"></i> <?php echo mb_substr($gAddress, 0, 30); ?></span>
                        </div>
                    <?php } ?>
                </div>
            </div>
        ...
        <div class="pull-right pos-relative-min-height-100">
            ...
            <ul class="md-experiences">
                <?php
                $listingExtraOptions = get_post_meta($doctor_id, 'mp_listing_extra_fields', true);
                $virtual_consult = get_post_meta($doctor_id, 'mp_listing_extra_fields_virtual_consult', true);
                $certified_doctor = get_post_meta($doctor_id, 'mp_listing_extra_fields_certified_doctor', true);
                $online_prescription = get_post_meta($doctor_id, 'mp_listing_extra_fields_online_prescription', true);
                $taking_new_patient = get_post_meta($doctor_id, 'mp_listing_extra_fields_taking_new_patient', true);
            ...
            <div class="mp-list-view-right-bottom-content">
                <?php
                if (mp_get_listing_status($doctor_id) == 'opened') {
                ...
                }elseif (mp_get_listing_status($doctor_id) == 'opened_next') {
                ...
                <div class="mp-profile-location-book-outer text-right">
                    <a href="<?php echo get_permalink($doctor_id); ?>" class="mp-profile-location-book"> <?php esc_html_e('Book Appoinment', 'medicalpro'); ?></a>
                </div>
            </div>
        </div>
    </div>
...
<div class="col-md-4 <?php echo esc_attr($adClass); ?> lp-grid-box-contianer grid_view_s2 grid_view2 card1 lp-grid-box-contianer1 listing-grid-view2-outer md-listing-outer" data-title="<?php echo get_the_title($doctor_id); ?>" data-postid="<?php echo $doctor_id; ?>"   data-lattitue="<?php echo esc_attr($latitude); ?>" data-longitute="<?php echo esc_attr($longitude); ?>" data-posturl="<?php echo get_the_permalink(); ?>">
    <?php if(is_page_template('template-favourites.php')){ ?>
        <div class="remove-fav md-close" data-post-id="<?php echo $doctor_id; ?>">
            <i class="fa fa-close"></i>
        </div>
    ...
    <div class="lp-grid-box">
        ...
        <div class="md-grid-box-thumb-container" >
            <div class="lp-grid-box-thumb">
                <div class="show-img">
                    <?php
                    if ( has_post_thumbnail()) {
                        $image = wp_get_attachment_image_src( get_post_thumbnail_id( $doctor_id), 'listingpro-blog-grid2' );
                        if(!empty($image[0])){
                ...
                <div class="hide-img listingpro-list-thumb">
                    <?php
                    if ( has_post_thumbnail()) {
                        $image = wp_get_attachment_image_src( get_post_thumbnail_id( $doctor_id), 'listingpro-blog-grid' );
                        if(!empty($image[0])){
                            echo "<a href='".get_the_permalink()."' >
																<img src='" . $image[0] . "' />
															</a>";
        ...
        <div class="lp-grid-desc-container clearfix">
                ...
                <div class="lp-grid-box-left">
                    <h4 class="lp-h4">
                        <a href="<?php echo get_the_permalink(); ?>">
                            <?php echo mb_substr(get_the_title($doctor_id), 0, 18); ?>
                        </a>
                    </h4>
            ...
            <div class="mp-profile-location-book-outer">
                <a href="<?php echo get_permalink($doctor_id);?>" class="mp-profile-location-book"> <?php echo esc_html_e('Book Appoinment', 'medicalpro'); ?></a>

            </div>
            <?php
            $openStatus = listingpro_check_time($doctor_id);
            $cats = get_the_terms( $doctor_id, 'location' );
