...
if (!function_exists('Medpro_listing_submit_init')) {

    function Medpro_listing_submit_init($hook)
    {
        wp_dequeue_script('listingpro-submit-listing');
        wp_register_script('medpro-submit-listing', plugins_url('/assets/js/submit-listing.js', plugin_dir_path(__FILE__)), '', '', true);
        wp_enqueue_script('medpro-submit-listing');
        wp_localize_script('medpro-submit-listing', 'ajax_medpro_submit_object', array(
            'ajaxurl' => admin_url('admin-ajax.php'),
        ));
    }
}
if (!is_admin()) {
    add_action('init', 'Medpro_listing_submit_init', 30);
}

/* ================================================================================= */
add_action('wp_ajax_medpro_submit_listing_ajax', 'medpro_submit_listing_ajax');
add_action('wp_ajax_nopriv_medpro_submit_listing_ajax', 'medpro_submit_listing_ajax');
if (!function_exists('medpro_submit_listing_ajax')) {

    function medpro_submit_listing_ajax()
    {
        ...
        if ($processSubmission == true) {
            if (isset($_POST['post_nonce_field']) && wp_verify_nonce($_POST['post_nonce_field'], 'post_nonce')) {
                ...
                $videocheckmp = $_POST['mp_lp_submit_listing_extra_fields']['virtual_consult'];
                if (!empty($videocheckmp)) {
                    if (empty($_POST['videoconsult'])) {
                        $errors['videoconsult'] = 'videoconsult';
                    }
                }
                        ...
                        if ($_FILES['listingfiles']['size'] != 0) {
                            ...
                            if (!empty($counterLimit) && !empty($totalGalCount)) {
                                if ($totalGalCount > $counterLimit) {
                                    ...
                                    $msg2 = esc_html__('Max. allowed images are ', 'medicalpro');
                                    $msg2 .= $counterLimit;
                                }
                            }
                            if (!empty($sizeLimit) && !empty($totalGalSize)) {
                                if ($totalGalSize > $sizeLimit) {
                                    $errors['listingfiles'] = 'listingfiles';
                                    $msg2 = esc_html__('Max. allowed images size is ', 'medicalpro');
                                    $msg2 .= $sizeLimit . '';
                                    $msg2 .= 'Mb';
                                }
                            }
                ...
                if (!empty($errors) && count($errors) > 0) {
                    $msg = esc_html__('Sorry! There is problem in your submission', 'medicalpro');
                    ...
                    if (isset($_POST['lp_form_fields_inn'])) {
                        if (isset($_POST['lp_form_fields_inn']['lp_feature']) && !empty($_POST['lp_form_fields_inn']['lp_feature'])) {
                            ...
                            $featuresID = array_map('intval', $featuresID);
                            $featuresID = array_unique($featuresID);
                    ...
                    if (isset($_POST['insurance'])) {
                        $lpcatsss = $_POST['insurance'];
                        if (!empty($lpcatsss) && is_array($lpcatsss)) {
                            $lp_listing_ins = $lpcatsss;
                        } else {
                            $lp_listing_ins = array($lpcatsss);
                        }
                    }
                    if (isset($_POST['award'])) {
                        $lpcatsss = $_POST['award'];
                        if (!empty($lpcatsss) && is_array($lpcatsss)) {
                            $lp_listing_awd = $lpcatsss;
                        } else {
                            $lp_listing_awd = array($lpcatsss);
                        }
                    }

                    //new code 1.4
                    if (!empty($lp_listing_ins) && is_array($lp_listing_ins)) {
                        $lp_listing_ins = array_map('intval', $lp_listing_ins);
                        $lp_listing_ins = array_unique($lp_listing_ins);
                    }

                    if (!empty($lp_listing_awd) && is_array($lp_listing_awd)) {
                        $lp_listing_awd = array_map('intval', $lp_listing_awd);
                        $lp_listing_awd = array_unique($lp_listing_awd);
                    }
                    //new code 1.4
                    ...
                    if (is_user_logged_in()) {
                        $post_information = array(
                            ...
                            'tax_input' => array(
                                ...
                                'medicalpro-insurance' => $lp_listing_ins,
                                'medicalpro-award' => $lp_listing_awd,
                                'features' => $featuresID,
                                'list-tags' => $tags,
                            ),
                            'post_status' => 'pending'
                        ...
                        $hospital_primary = isset($_POST['medicalpro_primary']) ? $_POST['medicalpro_primary'] : '';
                        if (!empty($hospital_primary)) {
                            $phospitalID = $hospital_primary;
                            $Phospital = get_term_by('term_id', $phospitalID, 'medicalpro-hospital');
                            $PhospitalID = $Phospital->term_id;
                            $listLatitude = get_term_meta($PhospitalID, 'latitude', true);
                            $listLongitude = get_term_meta($PhospitalID, 'longitude', true);
                            $listPhone = get_term_meta($PhospitalID, 'phone', true);
                            $gaddpress = get_term_meta($PhospitalID, 'address', true);
                            ...
                            listing_set_metabox('mp_primary_hospital', $hospital_primary, $postID);
                        }
                        ...
                        $videoConsultURL = sanitize_text_field($_POST['videoconsult']);
                        listing_set_metabox('videoconsult', $videoConsultURL, $postID);
                        if (isset($_POST['mp_lp_submit_listing_extra_fields']['virtual_consult']) && !empty($_POST['mp_lp_submit_listing_extra_fields']['virtual_consult'])) {
                            $virtual_consult = $_POST['mp_lp_submit_listing_extra_fields']['virtual_consult'];
                            update_post_meta($postID, 'mp_listing_extra_fields_virtual_consult', $virtual_consult);
                        } else {
                            delete_post_meta($postID, 'mp_listing_extra_fields_virtual_consult');
                        }
                        if (isset($_POST['mp_lp_submit_listing_extra_fields']['online_prescription']) && !empty($_POST['mp_lp_submit_listing_extra_fields']['online_prescription'])) {
                            $online_prescription = $_POST['mp_lp_submit_listing_extra_fields']['online_prescription'];
                            update_post_meta($postID, 'mp_listing_extra_fields_online_prescription', $online_prescription);
                        } else {
                            delete_post_meta($postID, 'mp_listing_extra_fields_online_prescription');
                        }
                        if (isset($_POST['mp_lp_submit_listing_extra_fields']['taking_new_patient']) && !empty($_POST['mp_lp_submit_listing_extra_fields']['taking_new_patient'])) {
                            $taking_new_patient = $_POST['mp_lp_submit_listing_extra_fields']['taking_new_patient'];
                            update_post_meta($postID, 'mp_listing_extra_fields_taking_new_patient', $taking_new_patient);
                        } else {
                            delete_post_meta($postID, 'mp_listing_extra_fields_taking_new_patient');
                        }

                        do_action('medicalpro_save_listing_submission', $postID, $_POST, $_FILES);
                        ...
                        $msg = esc_html__('Success! Submission is successful', 'medicalpro');
                        ...
                        if ($_POST['processLogin'] == 'no' || !isset($_POST['processLogin'])) {
                            ...
                            if (!empty($existinUserId)) {
                                $response = '<span class="email-exist-error">' . esc_html__("UserName already exists", "medicalpro") . '</span>';
                                $msg = esc_html__('Sorry! There is problem in your submission', 'medicalpro');
                            ...
                            if (email_exists($email) == true || username_exists($user_name) == true) {
                                $response = '<span class="email-exist-error">' . esc_html__("Email already exists", "medicalpro") . '</span>';
                                $msg = esc_html__('Sorry! There is problem in your submission', 'medicalpro');
                                die(json_encode(array('response' => 'failure', 'status' => $response, 'msg' => $msg)));
                            } else {
                                ...
                                $lp_listing_ins = $_POST['insurance'];
                                $lp_listing_ins = array_map('intval', $lp_listing_ins);
                                $lp_listing_ins = array_unique($lp_listing_ins);

                                $lp_listing_awd = $_POST['award'];
                                $lp_listing_awd = array_map('intval', $lp_listing_awd);
                                $lp_listing_awd = array_unique($lp_listing_awd);

                                wp_set_post_terms($postID, $lp_listing_ins, 'medicalpro-insurance');
                                wp_set_post_terms($postID, $lp_listing_awd, 'medicalpro-award');
                            ...
                            $user = wp_signon($creds, true);
                            if (is_wp_error($user)) {
                                $response = '<span class="invalid-cred-error">' . esc_html__("Invalid username or password", "medicalpro") . '</span>';
                                $msg = esc_html__('Sorry! There is problem in your submission', 'medicalpro');
                                die(json_encode(array('response' => 'failure', 'status' => $response, 'msg' => $msg)));
                            } else {
            ...
            if (isset($_POST['edit_nonce_field']) && wp_verify_nonce($_POST['edit_nonce_field'], 'edit_nonce')) {
                ...
                if (!empty($errors) && count($errors) > 0) {
                    $msg = esc_html__('Sorry! There is problem in listing submission', 'medicalpro');
                    die(json_encode(array('response' => 'fail', 'status' => $errors, 'msg' => $msg)));
                } else {
            ...
            $msg = esc_html__('Sorry! There is problem in your submission', 'medicalpro');
            $response = '<span class="email-exist-error error-msg">' . esc_html__("Please check captcha", "medicalpro") . '</span>';
