<?php
	...
	if (!function_exists('medicalpro_review_submit_init')) {
		function medicalpro_review_submit_init(){
		...
		if(!is_admin()){
			add_action('wp_enqueue_scripts', 'medicalpro_review_submit_init');
		}
	}
		...
		if($processReview==true){
			...
			if ( is_user_logged_in() ) {
				...
				if( $listing_author_id==$uid ){
					$res = json_encode(array('reviewID'=>$postID, 'status'=>esc_html__('Sorry! Listing owners can not post review on their listings!', 'medicalpro'), 'error'=>'already'));
					die($res);
				}
				elseif($hasReview !="1"){
					$post_information = array(
					...
					$res = json_encode(array('reviewID'=>$postID, 'status'=>esc_html__('Sorry! already posted on this listing!', 'medicalpro'), 'error'=>'already'));
					die($res);
				}
				...
				if( email_exists($user_email)==true ){
					$res = json_encode(array('reviewID'=>$postID, 'status'=>esc_html__('Email already exists! Please login or change email', 'medicalpro'), 'error'=>'email'));
					die($res);
				}
				else{
					...
					if(!empty($existinUserId)){
						$res = json_encode(array('reviewID'=>$postID, 'status'=>esc_html__('User name already exists! Please login or try a different one', 'medicalpro'), 'error'=>'email'));
						die($res);
					}
			...
			$dataRes['status']= esc_html__('Review has been submitted successfully', 'medicalpro');
			$res = json_encode(array('reviewID'=>$postID, 'status'=>esc_html__('Review has been submitted successfully', 'medicalpro')));
			...
			$res = json_encode(array('reviewID'=>$postID, 'status'=>esc_html__('Please check captcha', 'medicalpro'), 'error'=>'email', 'processReview'=>$processReview, 'keyResponse'=>$keyResponse));
		}
	...
	if(!function_exists('lp_reviews_interests')){
		function lp_reviews_interests(){
			...
			if(empty($reactionCookie)){
				...
				$status = esc_html__('Thanks for reacting', 'medicalpro');
				...
				$status = esc_html__('You already reacted', 'medicalpro');
